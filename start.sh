...
python -c "import pathlib; from django.conf import settings; p=pathlib.Path(settings.STATIC_ROOT); print('STATIC_ROOT exists after collectstatic:', p.exists()); print('STATIC_ROOT=', settings.STATIC_ROOT); print('STATIC_ROOT listing (head):', list(p.glob('**/*'))[:10] if p.exists() else 'MISSING')" || true
